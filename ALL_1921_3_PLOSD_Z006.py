...
NIS_All_Clean_Merge_2 = NIS_All_Clean_Merge_2[NIS_All_Clean_Merge_2['Z006'] == 1]
print("Length of Z006:", len(NIS_All_Clean_Merge_2))
...
binary_columns = ['KID','C9100','C9102'] # Exclude the C9101 (Remission)
...
for day in PRDay_list:
    ...
    # Filter data based on day and outcome non-NaN values
    ...
    for outcome in Response_column:
        ...
        rf_probs = rf.predict_proba(X_test_selected_rf)[:, ] # for the PLOSD
        gb_probs = gb.predict_proba(X_test_selected_gb)[:, ] # for the PLOSD
        #
        # Add the prediction value
        # Create dynamic column names
        ...
        # ############## For 4 levels outcome ##########################
        # For each class, store probabilities as new columns in y_test
        for class_label in range(rf_probs.shape[1]):  # Loop over classes
            rf_prob_name = f"RF_Prob_Class_{class_label}_{outcome}"  # Dynamic column name for RF
            gb_prob_name = f"GB_Prob_Class_{class_label}_{outcome}"  # Dynamic column name for GB

            # Add probabilities for the current class
            y_test[rf_prob_name] = rf_probs[:, class_label]
            y_test[gb_prob_name] = gb_probs[:, class_label]
        ...
        auc_rf = roc_auc_score(y_test[outcome], rf_probs, multi_class='ovr', average='macro') # for PLOSD
        ...
        auc_gb = roc_auc_score(y_test[outcome], gb_probs, multi_class='ovr', average='macro') # for PLOSD
    ...
    filename = fr"C:\Users\Jiahui\PycharmProjects\NIS\Outcome\PLOSD\Prediction_summary_Z006_{day}.csv"
    outcome_df.to_csv(filename, index=False)

    # Save the DataFrame to a CSV file
    filename1 = fr"C:\Users\Jiahui\PycharmProjects\NIS\Outcome\PLOSD\Prediction_data_output_Z006_{day}.csv"
