...
list_feature = ["Day", "Selected_feature_rf", "Feature_importance_rf"]
...
print(Prediction_summary_2['Selected_feature_rf'].dtype)
...
# Sort values
df_sorted = df_exploded.sort_values(by=['Day', 'Feature_importance_rf'], ascending=[True, False])

# Save sorted data
df_sorted.to_csv(r"C:\Users\Jiahui\PycharmProjects\NIS\TCprediction_comb\Sorted_Feature_outcome.csv", index=False)
...
top_features = df_sorted.groupby("Day").apply(lambda x: x.nlargest(22, "Feature_importance_rf")).reset_index(drop=True)
...
# Get the top 20 features for each day
top_features_f = df_filtered.groupby("Day").apply(lambda x: x.nlargest(22, "Feature_importance_rf")).reset_index(drop=True)
